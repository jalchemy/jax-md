...
def load_lammps_eam_parameters_alloy(
  ...
  def parse_embedding_charge_block(start, end):
    ...
    # print(f"embedding block[:10]: {block[:10]}")
    # print(f"embedding block.shape: {block.shape}")
    # print(f"{num_drho, num_drho + num_dr = }")
    # print(
    #     f"charge block[num_drho : num_drho + num_dr]: {block[num_drho : num_drho + num_dr]}"
    # )
    # print(f"embedding block[-1]: {block[-1]}")

    # Special case for some setfl files that seem to include (num_dr + 1) + (num_drho + 1) values in the
    # charge-embdeeing block. We will omit the final values the charge and embedding parts (separately)
    if block.shape[0] > num_dr + num_drho:
      embedding_fn = interpolate.spline(block[:num_drho], drho)
      charge_fn = interpolate.spline(
        block[num_drho + 1 : num_drho + num_dr + 1], dr
      )
    else:
      embedding_fn = interpolate.spline(block[:num_drho], drho)
  ...
  def parse_pairwise_block(start, end):
    ...
    # print(f"{block.shape = }")
    # print(f"pairwise block[0] = {block[0]}")
    # print(f"pairwise block[-1] = {block[-1]}")
  ...
  for i in range(species_count):
    ...
    block_length = (
      int(
        jnp.ceil(
          (num_drho) / values_per_line,
        )
        + jnp.ceil(
          (num_dr) / values_per_line,
        )
      )
      + 1
    )
    ...
    # Parse embedding and density blocks for element i. Pass in line numbers for start and end of blocks e.g.
    # [6, 1007)
    # [1007, 2008)
    # [2008, 3009)
    # print(f"{data_start + i * block_length = }")
    ...
    for j in range(i + 1):
      ...
      pairwise_block_length = int(jnp.ceil(num_dr / values_per_line))
...
def test_fork():
  return 'JAX MD is running in the local, forked repo'


# def species_mask(arr, mask):
...
def eam_alloy_neighbor_list(
  ...
  **neighbor_kwargs,
  ...
  def energy_fn(R, neighbor, *, species, idxs=None, **kwargs):
    ...
    if idxs is not None:
      neighbor_self_mask = partition.neighbor_list_mask(
        neighbor, mask_self=True
      )[idxs]
    else:
      neighbor_self_mask = partition.neighbor_list_mask(
        neighbor, mask_self=True
    ...
    if neighbor.format is partition.Dense:
      ...
      for i in range(species_count):
        ...
        if idxs is not None:
          ...
          Ri_nb_mask = jnp.where(species[neighbor.idx[idxs]] == i, True, False)
        else:
          Ri_mask = jnp.where(species == i, True, False)
