...
def load_lammps_tersoff_parameters(file: TextIO) -> Array:
  ...
  for line in file.read().split("\n"):
    ...
    if nwords != params_per_line:
      raise ValueError("Incorrect format: %d not in %d" % (nwords, params_per_line))
    else:
      skip = False
  ...
  for i in range(species_count):
    ...
    for j in range(i + 1):
      ...
      pairwise_fn = parse_pairwise_block(
        int(pairwise_data_start + (i * (i + 1) / 2 + j) * pairwise_block_length),
        int(pairwise_data_start + ((i * (i + 1) / 2 + j) + 1) * pairwise_block_length),
...
def test_fork():
  print("JAX MD is running in the local, forked repo")
    ...
    for i in range(species_count):
      ...
      for j in range(i + 1):
        ...
        if j == i:
          ...
          charge_arr = smap._diagonal_mask(charge_fns[j](dr)) * Rj_mask * cutoff_mask
    ...
    if neighbor.format is partition.Dense:
      ...
      for i in range(species_count):
        ...
        for j in range(i + 1):
          ...
          pairwise_mask = Ri_mask[:, None] * Rj_nb_mask + Rj_mask[:, None] * Ri_nb_mask
  ...
  **neighbor_kwargs,
  ...
  def energy_fn(R, neighbor, **kwargs):
    ...
    if neighbor.format is partition.Dense:
      ...
      embedding_energy = embedding_fn(total_charge)
      pairwise_energy = util.high_precision_sum(pairwise_fn(dr) * self_mask, axis=1)
      ...
      embedding_energy = embedding_fn(total_charge)
      pairwise_energy = ops.segment_sum(pairwise_fn(dr) * self_mask, neighbor.idx[0], N)
    else:
      raise NotImplementedError(
        "EAM potential not implemented for OrderedSparse neighbor lists."
      )

    return util.high_precision_sum(embedding_energy + pairwise_energy / 2.0, axis=axis)
...
class EnergyGraphNet(hk.Module):
  ...
  def __init__(
    ...
    self._format = format
    self._graph_net = nn.GraphNetEncoder(n_recurrences, mlp_sizes, mlp_kwargs, format)
